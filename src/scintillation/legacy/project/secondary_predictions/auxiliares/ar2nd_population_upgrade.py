...
def upgrade_population_csv(population_csv: str | Path, *, force: bool = False) -> UpgradeReport:
    ...
    has_required = all(column in df.columns for column in _REQUIRED_COLUMNS)
    if not force and has_required and marker.is_file():
        return UpgradeReport(population_csv, 0, ())

    level_dir = population_csv.parent.parent / "csv"
    if not level_dir.is_dir():
        # Compact mode deliberately stores only the aggregate population table.
        # Tables generated by the current population registry already use the
        # disjoint 11.5--11.7, 11.7--12.0 and 12.0--100 eV bins.
        if has_required:
            marker.write_text(
                "Ar2nd bins: trusted compact aggregate; current registry convention\n",
                encoding="utf-8",
            )
            return UpgradeReport(population_csv, 0, ())
