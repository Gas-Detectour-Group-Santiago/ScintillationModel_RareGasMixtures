...
def _load_ocw_recipe(recipe_id: str) -> OCWBandConfig:
    project_root = Path(os.environ.get("SCINTILLATION_ROOT", PROJECT_ROOT))
    path = project_root / "config" / "ocw.csv"
    frame = pd.read_csv(path)
    selected = frame.loc[(frame["recipe_id"].astype(str) == recipe_id) &
                         frame["enabled"].astype(str).str.lower().isin({"1", "true", "yes", "on"})]
    if selected.empty:
        raise KeyError(f"OCW recipe {recipe_id!r} not found in {path}")
    first = selected.iloc[0]
    def optional(value):
        return None if pd.isna(value) or str(value).strip() == "" else float(value)
    rules = tuple(OCWParameterRule(
        str(row.parameter), low_factor=float(row.low_factor), high_factor=float(row.high_factor),
        optimum_factor=optional(row.optimum_factor), low_add=float(row.low_add), high_add=float(row.high_add),
        optimum_add=optional(row.optimum_add), clip_min=optional(row.clip_min), clip_max=optional(row.clip_max),
    ) for row in selected.itertuples(index=False))
    return OCWBandConfig(id=recipe_id, label=str(first["label"]),
                         use_corners=str(first["use_corners"]).lower() in {"1", "true", "yes", "on"},
                         rules=rules)
...
ARCF4_VISIBLE_OCW = _load_ocw_recipe("ArCF4_visible_OCW")
...
ARCF4_UV_OCW = _load_ocw_recipe("ArCF4_UV_OCW")
...
ARCF4_CF4_IONIC_VUV_OCW = _load_ocw_recipe("ArCF4_CF4_ionic_VUV_OCW")
...
ARCF4_IR_OCW = _load_ocw_recipe("ArCF4_IR_OCW_x2")
...
from scintillation.plotting.recipe_config import (
    load_secondary_metadata_plots as _load_secondary_metadata_plots_csv,
    load_secondary_multiband_plots as _load_secondary_multiband_plots_csv,
)


def _plot_normalization(normalization_id: str) -> NormalizationConfig:
    key = str(normalization_id).strip()
    if key in {"secondary_ArCF4_ne", "secondary_ArCF4", "ArCF4"}:
        return ARCF4_SECONDARY_NORM_NE
    if key in {"fixed_norm", "Ar2nd_absolute", "absolute"}:
        return AR2ND_SECONDARY_NORM_NE
    raise KeyError(f"Normalización secundaria desconocida en config/plots/secondary.csv: {key!r}")


def _plot_ocw(recipe_id: str) -> OCWBandConfig | None:
    key = str(recipe_id).strip()
    return None if not key else _load_ocw_recipe(key)


def secondary_metadata_plots(which: str = "paper") -> list[MetadataPlotConfig]:
    key = str(which).strip().lower()
    if key in {"paper", "config_paper"}:
        return _load_secondary_metadata_plots_csv(
            PROJECT_ROOT,
            "metadata",
            curve_cls=MetadataCurveConfig,
            plot_cls=MetadataPlotConfig,
            selection_cls=SecondarySelection,
        )
    if key in {"comparation", "comparison", "config_comparation"}:
        return []
    raise ValueError("which debe ser 'paper' o 'comparation'.")
...
def secondary_multiband_plots() -> list[MultiBandPlotConfig]:
    return _load_secondary_multiband_plots_csv(
        PROJECT_ROOT,
        "paper",
        curve_cls=BandCurveConfig,
        combined_cls=CombinedBandCurveConfig,
        plot_cls=MultiBandPlotConfig,
        selection_cls=SecondarySelection,
        experimental_series_cls=ExperimentalSeries,
        normalization_lookup=_plot_normalization,
        ocw_lookup=_plot_ocw,
    )
