...
from scintillation.plotting.recipe_config import read_plot_rows, split_values, grid_from_row, as_limits, as_text, output_path, fit_correlation_enabled
...
def fit_once(
    ...
    *,
    ...
    # Seeds are assembled from independent IR fits whose normalisations and
    # quenching parameters can legitimately sit outside the joint-fit bounds.
    # Project the starting point into the open feasible interval; this changes
    # only the optimiser seed, never the model or the fitted bounds.
    z_start = np.clip(
...
def plot_fit_diagnostics(
    ...
    recipes = read_plot_rows("fits", PROJECT_ROOT)
    recipes = recipes.loc[(recipes["fit_id"] == FIT_NAME) & (recipes["plot_type"] == "joint_ir")]
    cmap = plt.get_cmap("viridis")
    for _, recipe in recipes.iterrows():
        dataset_id = as_text(recipe.get("dataset_id"))
        mixture, line = dataset_id.split("_", 1)
        x_col = "fCF4" if mixture == "ArCF4" else "fN2"
        fig, ax = plt.subplots(figsize=(6.4, 4.5))
        df = frames[(mixture, line)]
        grid_fraction = grid_from_row(recipe)
        grid_percent = grid_fraction * float(recipe.get("x_plot_factor") or 100.0)
        pressures = split_values(recipe.get("pressures_bar"), cast=float) or PRESSURES
        for ip, pressure in enumerate(pressures):
            color = cmap((ip + 0.7) / (len(pressures) + 0.4))
            y = pd.to_numeric(df[pressure_label(pressure)], errors="coerce").to_numpy(dtype=float)
            # Fit figures always show statistical uncertainties on the points.
            e = pd.to_numeric(df[stat_error_label(pressure)], errors="coerce").to_numpy(dtype=float)
            ...
            x_plot = x_percent[valid].copy(); x_plot[np.isclose(x_plot, 0.0)] = 1e-3
            ax.errorbar(x_plot, y[valid], yerr=e[valid], fmt="o", ms=3.5, capsize=2, color=color, alpha=0.8)
            y_th = theory_yield_joint(central_x, degrad[mixture], grid_fraction, pressure, mixture=mixture, line=line)
            ax.plot(grid_percent, y_th, lw=1.7, color=color, label=f"{pressure:g} bar")
        ax.set_xscale(as_text(recipe.get("xscale"), "log")); ax.set_yscale(as_text(recipe.get("yscale"), "log"))
        xlim=as_limits(recipe,"x"); ylim=as_limits(recipe,"y")
        if xlim: ax.set_xlim(*xlim)
        if ylim: ax.set_ylim(*ylim)
        ax.set_xlabel(as_text(recipe.get("xlabel"))); ax.set_ylabel(as_text(recipe.get("ylabel")))
        ax.set_title(as_text(recipe.get("title"))); ax.legend(ncol=int(recipe.get("legend_ncol") or 2), fontsize=8)
        fig.tight_layout(); destination=output_path(recipe, PROJECT_ROOT); destination.parent.mkdir(parents=True,exist_ok=True)
        fig.savefig(destination,bbox_inches="tight"); plt.close(fig)
...
def export_joint_toy_correlations(
    ...
    if not fit_correlation_enabled(PROJECT_ROOT, FIT_NAME, default=True):
        return metadata
