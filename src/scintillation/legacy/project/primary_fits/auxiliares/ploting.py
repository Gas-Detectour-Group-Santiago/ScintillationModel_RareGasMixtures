...
from plot_style import LEGEND, palette, setup_style
...
def plot_fit_vs_experiment_by_pressure(
    ...
    *,
    ...
    if err_patterns is None:
        # Fit panels always display the statistical uncertainty of the
        # experimental points.  Systematic uncertainties enter through the
        # nuisance/toy propagation, not through the point error bars.
        err_patterns = [
            "ErrStat {col}",
            "ErrStat_{col}",
            "{col} ErrStat",
    ...
    for (p, col), color in zip(pressure_cols, colors):
        ...
        yerr = np.abs(df_exp[err_col].to_numpy(dtype=float)) if err_col is not None else None

        mask_exp = _valid_mask_for_plot(
            x_exp_plot,
            y_exp,
            xscale=xscale,
            yscale=yscale,
        ...
        if yerr is not None:
            mask_exp &= np.isfinite(yerr) & (yerr > 0)
            yerr_plot = yerr[mask_exp]
        else:
            yerr_plot = None
