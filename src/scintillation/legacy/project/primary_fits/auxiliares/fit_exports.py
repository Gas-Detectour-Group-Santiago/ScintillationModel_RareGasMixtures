...
def plot_parameter_correlation(
    ...
    """Export a style-consistent PDF heatmap for a correlation matrix."""
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    n = len(corr.columns)
    labels = list(corr.columns) if labels is None else labels
    masked = np.ma.masked_invalid(values)
    fig_size = max(5.0, 0.58 * n + 2.1)
    fig, ax = plt.subplots(figsize=(fig_size, fig_size))
    image = ax.imshow(masked, vmin=-1.0, vmax=1.0, cmap="coolwarm", aspect="equal")
    for i in range(n):
        for j in range(n):
            value = values[i, j]
            text = "--" if i == j and not np.isfinite(value) else (f"{value:.2f}" if np.isfinite(value) else "")
            if text:
    ...
    cbar.set_label(r"Correlation coefficient $\rho_{ij}$")
    fig.tight_layout()
    fig.savefig(path)
    plt.close(fig)
