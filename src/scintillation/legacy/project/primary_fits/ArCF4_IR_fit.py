...
# Production plot selection is declarative. The hard-coded PLOTS list above is
# retained only as a documented fallback for old standalone copies.
from scintillation.plotting.recipe_config import fit_correlation_enabled, load_fit_plot_specs

CONFIG = replace(
    CONFIG,
    plots=load_fit_plot_specs(
        PROJECT_ROOT,
        CONFIG.name,
        plot_spec_cls=PlotSpec,
        fallback=CONFIG.plots,
    ),
    make_correlation_plot=fit_correlation_enabled(
        PROJECT_ROOT, CONFIG.name, CONFIG.make_correlation_plot
    ),
)
