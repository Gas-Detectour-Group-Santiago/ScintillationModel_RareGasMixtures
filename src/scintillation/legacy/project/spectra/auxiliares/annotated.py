...
def _plot_one_annotated(
    ...
    *,
    ...
    pdf_path = destination or (outdir / "plots" / "annotated" / str(spec["output_pdf"]))
...
def run_annotated_figures(
    project_root: Path,
    outdir: Path,
    recipes: pd.DataFrame | None = None,
) -> dict[str, Path]:
    """Render the annotated spectrum rows selected in ``spectra.csv``.

    Annotation peak definitions stay as reusable scientific profiles in this
    module, while title, wavelength range, output path and activation are
    controlled by one CSV row per PDF.
    """
    from scintillation.plotting.recipe_config import as_float, as_text, output_path

    _configure_annotated_style()
    raw_cache: dict[str, pd.DataFrame] = {}
    out: dict[str, Path] = {}
    profile_lookup = {str(spec["name"]): spec for spec in ANNOTATED_SPECS}

    if recipes is None:
        iterable = [
            {
                "plot_id": name,
                "annotation_profile": name,
                "title": spec["title"],
                "wavelength_min_nm": spec["x_range_nm"][0],
                "wavelength_max_nm": spec["x_range_nm"][1],
                "output": str(Path("spectra") / "plots" / "annotated" / spec["output_pdf"]),
            }
            for name, spec in profile_lookup.items()
        ]
    else:
        iterable = [row for _, row in recipes.iterrows()]

    for row in iterable:
        profile_id = as_text(row.get("annotation_profile"), as_text(row.get("plot_id")))
        if profile_id not in profile_lookup:
            raise KeyError(f"Unknown annotation profile {profile_id!r}")
        spec = dict(profile_lookup[profile_id])
        spec["name"] = as_text(row.get("plot_id"), profile_id)
        spec["title"] = as_text(row.get("title"), spec["title"])
        spec["x_range_nm"] = (
            float(as_float(row.get("wavelength_min_nm"), spec["x_range_nm"][0])),
            float(as_float(row.get("wavelength_max_nm"), spec["x_range_nm"][1])),
        )
        ...
        destination = output_path(row, project_root) if recipes is not None else project_root / str(row["output"])
        out[str(spec["name"])] = _plot_one_annotated(
            outdir, spec, wavelength, intensity, destination=destination
        )
