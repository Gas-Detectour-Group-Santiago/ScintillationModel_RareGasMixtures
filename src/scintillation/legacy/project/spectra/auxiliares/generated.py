...
def _cache_covers_requested_grid(df: pd.DataFrame, components: tuple[str, ...]) -> bool:
    if df.empty or not {"pressure_bar", "concentration_percent"}.issubset(df.columns):
        return False
    pressures = pd.to_numeric(df["pressure_bar"], errors="coerce").dropna().unique()
    concentrations = pd.to_numeric(df["concentration_percent"], errors="coerce").dropna().unique()
    component_values = set(df["component"].astype(str).unique()) if "component" in df else {"total"}
    return (
        all(np.isclose(pressures, float(value)).any() for value in cfg.GENERATED_PRESSURES_BAR)
        and all(np.isclose(concentrations, float(value)).any() for value in cfg.GENERATED_CONCENTRATIONS_PERCENT)
        and set(components).issubset(component_values)
    )


def _load_or_build_generated(
    builder: GeneratedBuilder, gas: str, outdir: Path, *, amplied: bool, components: tuple[str, ...]
) -> pd.DataFrame:
    stem = f"{gas}_spectra_generated_amplied" if amplied else f"{gas}_generated_spectra"
    compressed = outdir / "csv" / f"{stem}.csv.gz"
    legacy = outdir / "csv" / f"{stem}.csv"
    recompute = os.environ.get("RECOMPUTE_SPECTRA", "0").lower() in {"1", "true", "yes", "on"}

    if not recompute:
        for candidate in (compressed, legacy):
            if not candidate.exists():
                continue
            # Generated spectral grids can contain millions of rows.  Load only
            # columns used by the renderer and use compact dtypes so cached
            # products remain fast and memory-bounded during run_products.
            header = tuple(pd.read_csv(candidate, nrows=0).columns)
            wanted = tuple(
                column
                for column in (
                    "gas_mixture",
            ...
            dtype_map = {
                "gas_mixture": "category",
                "pressure_bar": "float32",
                "concentration_percent": "float32",
                "component": "category",
                "wavelength_nm": "float32",
                "intensity_ph_MeV_nm": "float32",
            }
            cached = pd.read_csv(
                candidate,
                usecols=list(wanted),
                dtype={key: value for key, value in dtype_map.items() if key in wanted},
            )
            if _cache_covers_requested_grid(cached, components):
                if candidate == legacy:
                    ensure_parent(compressed)
                    cached.to_csv(compressed, index=False, compression="gzip")
                    legacy.unlink(missing_ok=True)
                print(f"[spectra] {stem}: using cache {compressed}")
                return cached

    label = "generated amplied" if amplied else "generated"
    print(f"[spectra] {label} {gas}")
    df = builder.build_gas(gas, amplied=amplied)
    if "component" in df and components:
        df = df.loc[df["component"].astype(str).isin(components)].copy()
    ensure_parent(compressed)
    df.to_csv(compressed, index=False, compression="gzip")
    legacy.unlink(missing_ok=True)
    print(f"[spectra] CSV.GZ: {compressed}")
    return df


def build_generated_spectra(
    project_root: Path,
    outdir: Path,
    gases: tuple[str, ...] | list[str] | None = None,
    components: tuple[str, ...] = ("total",),
) -> dict[str, pd.DataFrame]:
    builder = GeneratedBuilder(project_root)
    selected = tuple(gases) if gases is not None else tuple(cfg.GASES)
    return {
        gas: _load_or_build_generated(builder, gas, outdir, amplied=False, components=components)
        for gas in selected
    }


def build_generated_amplied_spectra(
    project_root: Path,
    outdir: Path,
    gases: tuple[str, ...] | list[str] | None = None,
    components: tuple[str, ...] = ("total",),
    ...
    selected = tuple(gases) if gases is not None else tuple(cfg.GASES)
    return {
        gas: _load_or_build_generated(builder, gas, outdir, amplied=True, components=components)
        for gas in selected
    }
