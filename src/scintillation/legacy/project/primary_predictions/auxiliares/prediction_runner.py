...
class PredictionRunner:
    def __init__(self, project_root: Path, adapters: dict[str, object], overlays: list[ExperimentalOverlay] | None = None):
        ...
        # The same physical points are used by several output tables and
        # normalisation comparisons. Cache central values and toy samples for
        # the lifetime of one products run so the expensive model evaluation
        # is performed only once per unique point/normalisation.
        self._point_central_cache: dict[tuple[object, ...], float] = {}
        self._point_sample_cache: dict[tuple[object, ...], np.ndarray] = {}
    ...
    @staticmethod
    def _normalization_key(normalization: NormalizationConfig) -> tuple[object, ...]:
        return (
            normalization.mode,
            normalization.reference_fit_name or "",
            normalization.fixed_value,
            normalization.output_scale,
            normalization.output_unit,
            bool(normalization.propagate_nnorm),
        )

    def _point_key(self, point: PredictionPoint) -> tuple[object, ...]:
        concentration = point.concentration
        if isinstance(concentration, np.ndarray):
            concentration_key: object = tuple(np.asarray(concentration, dtype=float).ravel().tolist())
        elif isinstance(concentration, (list, tuple)):
            concentration_key = tuple(concentration)
        else:
        ...
        return (
            ...
            self._normalization_key(point.normalization),
        )

    def evaluate_point_samples(
        self,
        point: PredictionPoint,
        toy_params: np.ndarray,
        *,
        sample_kind: str,
    ) -> np.ndarray:
        ...
        cache_key = (*self._point_key(point), sample_kind)
        cached = self._point_sample_cache.get(cache_key)
        if cached is not None:
            return cached.copy()

        rows = []
        for params in self._progress(toy_params, total=len(toy_params), desc=f"{point.id} {sample_kind} toys"):
            try:
                rows.append(float(np.ravel(self.evaluate_point(point, params))[0]))
            except Exception:
                rows.append(np.nan)
        result = np.asarray(rows, dtype=float)
        self._point_sample_cache[cache_key] = result
        return result.copy()
    ...
    def build_point_table(self, points: list[PredictionPoint]) -> pd.DataFrame:
        ...
        for point in points:
            ...
            point_key = self._point_key(point)
            if point_key not in self._point_central_cache:
                self._point_central_cache[point_key] = float(np.ravel(self.evaluate_point(point, product.central))[0])
            central = self._point_central_cache[point_key]
            stat_samples = self.evaluate_point_samples(point, product.stat_toys, sample_kind="stat")
            syst_samples = self.evaluate_point_samples(point, product.syst_toys, sample_kind="syst")
    ...
    def run_values_by_normalization(
        ...
        *,
        ...
        if csv_path.exists() and not overwrite:
            print(f"[primary_predictions] tabla sin errores {stem}: usando caché {csv_path}")
    ...
    def run_normalization_comparison_points(
        ...
        *,
        ...
        csv_path = self.predictions_dir / f"{stem}.csv"
        if csv_path.exists() and not overwrite:
            print(f"[primary_predictions] tabla comparación {stem}: usando caché {csv_path}")
            df = pd.read_csv(csv_path)
        else:
    ...
    def run_points(
        ...
        *,
        ...
        csv_path = self.predictions_dir / f"{stem}.csv"
        if csv_path.exists() and not overwrite:
            print(f"[primary_predictions] tabla {stem}: usando caché {csv_path}")
            df = pd.read_csv(csv_path)
        else:
