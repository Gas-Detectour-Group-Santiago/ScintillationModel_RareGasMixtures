...
from scintillation.plotting.recipe_config import (  # noqa: E402
    as_float,
    as_int,
    as_limits,
    as_text,
...
def _recipe_row(project_root: Path, plot_id: str) -> pd.Series:
    rows = read_plot_rows("primary", project_root)
    selected = rows.loc[rows["plot_id"].astype(str) == plot_id]
    if selected.empty:
        raise KeyError(f"No active primary plot recipe {plot_id!r}")
    return selected.iloc[0]


def _selected_ids(recipe: Mapping[str, object], key: str, fallback: Sequence[str]) -> tuple[str, ...]:
    values = split_values(recipe.get(key), cast=str)
    return values or tuple(fallback)


def _apply_recipe_axes(
    ax, recipe: Mapping[str, object], *, default_xlabel: str, default_ylabel: str, default_title: str = ""
) -> None:
    ax.set_xscale(as_text(recipe.get("xscale"), "linear"))
    ax.set_yscale(as_text(recipe.get("yscale"), "linear"))
    xlim = as_limits(recipe, "x"); ylim = as_limits(recipe, "y")
    if xlim: ax.set_xlim(*xlim)
    if ylim: ax.set_ylim(*ylim)
    ax.set_xlabel(as_text(recipe.get("xlabel"), default_xlabel))
    ax.set_ylabel(as_text(recipe.get("ylabel"), default_ylabel))
    title = as_text(recipe.get("title"), default_title)
    if title:
        ax.set_title(title)
...
def _series_handles(
    channels: Sequence[EnergyChannel] = CHANNELS,
    particles: Sequence[str] = tuple(PARTICLE_STYLE),
    ...
    particle_handles = [
        ...
        for style in (PARTICLE_STYLE[particle],)
    ]
    return channel_handles, particle_handles


def _combined_series_handles(
    channels: Sequence[EnergyChannel] = CHANNELS,
    particles: Sequence[str] = tuple(PARTICLE_STYLE),
) -> list[Line2D]:
    channel_handles, particle_handles = _series_handles(channels, particles)
    spacer = Line2D([], [], linestyle="none", marker=None, color="none", label="Incident type")
    return [*channel_handles, spacer, *particle_handles]


def _plot_absolute(simulated: pd.DataFrame, dense: pd.DataFrame, output: Path, recipe: Mapping[str, object]) -> None:
    fig, ax = plt.subplots(figsize=FIGSIZE_WIDE)
    colors = _channel_colors()
    selected_channel_ids = _selected_ids(recipe, "component", [channel.id for channel in CHANNELS])
    selected_particle_ids = _selected_ids(recipe, "particles", list(PARTICLE_STYLE))
    channels = [channel for channel in CHANNELS if channel.id in selected_channel_ids]
    for channel in channels:
        color = colors[channel.id]
        for particle in selected_particle_ids:
            style = PARTICLE_STYLE[particle]
            points = simulated.loc[(simulated["channel"] == channel.id) & (simulated["particle"] == particle)].sort_values("energy_kev")
            curve = dense.loc[(dense["channel"] == channel.id) & (dense["particle"] == particle)]
            if not curve.empty:
                ax.plot(curve["energy_kev"], curve["yield_ph_MeV"], color=color, linestyle=style["linestyle"], linewidth=LINEWIDTH_MAIN)
            if not points.empty:
                ax.plot(points["energy_kev"], points["yield_ph_MeV"], linestyle="none", marker=style["marker"], color=color, markersize=MARKERSIZE + 0.5, markeredgecolor="white", markeredgewidth=0.45)
    _apply_recipe_axes(ax, recipe, default_xlabel="Incident energy [keV]", default_ylabel=r"Primary yield [ph MeV$^{-1}$]")
    ax.legend(
        handles=_combined_series_handles(channels, selected_particle_ids),
        loc=as_text(recipe.get("legend_loc"), "upper right"),
        ncol=int(as_int(recipe.get("legend_ncol"), 1) or 1),
        bbox_to_anchor=(0.985, 0.985), bbox_transform=ax.transAxes,
        **_boxed_legend_kwargs(handlelength=2.1, fontsize=as_float(recipe.get("legend_fontsize"), 9.4)),
    )
    apply_axis_style(ax); ax.margins(x=0.05, y=0.12)
...
def _plot_relative(simulated: pd.DataFrame, dense: pd.DataFrame, output: Path, recipe: Mapping[str, object]) -> None:
    ...
    for channel in channels:
        ...
        for particle in selected_particle_ids:
            ...
            curve = dense.loc[(dense["channel"] == channel.id) & (dense["particle"] == particle)]
            if not curve.empty:
                ax.plot(curve["energy_kev"], curve["yield_ph_MeV"] / reference, color=color, linestyle=style["linestyle"], linewidth=LINEWIDTH_MAIN)
            if not points.empty:
                ax.plot(points["energy_kev"], points["yield_ph_MeV"] / reference, linestyle="none", marker=style["marker"], color=color, markersize=MARKERSIZE + 0.5, markeredgecolor="white", markeredgewidth=0.45)
    ax.axhline(1.0, color="0.65", linewidth=0.8, zorder=0)
    _apply_recipe_axes(ax, recipe, default_xlabel="Incident energy [keV]", default_ylabel=r"$Y(E)/Y_{\mathrm{X\ ray}}(12\,\mathrm{keV})$")
    ax.legend(
        handles=_combined_series_handles(channels, selected_particle_ids),
        loc=as_text(recipe.get("legend_loc"), "upper right"),
        ncol=int(as_int(recipe.get("legend_ncol"), 1) or 1),
        bbox_to_anchor=(0.985, 0.985), bbox_transform=ax.transAxes,
        **_boxed_legend_kwargs(fontsize=as_float(recipe.get("legend_fontsize"), 8.8), handlelength=2.0),
    )
    apply_axis_style(ax); ax.margins(x=0.05, y=0.12)
    fig.savefig(output); plt.close(fig)
...
def _plot_individual(simulated: pd.DataFrame, dense: pd.DataFrame, channel: EnergyChannel, output: Path, recipe: Mapping[str, object]) -> None:
    ...
    for particle in selected_particle_ids:
        ...
        curve = dense.loc[(dense["channel"] == channel.id) & (dense["particle"] == particle)]
        if not curve.empty:
            ax.plot(curve["energy_kev"], curve["yield_ph_MeV"], color=color, linestyle=style["linestyle"], linewidth=LINEWIDTH_MAIN, label=style["label"])
        elif not points.empty:
            ax.plot([], [], color=color, linestyle=style["linestyle"], marker=style["marker"], label=style["label"])
        if not points.empty:
            ax.plot(points["energy_kev"], points["yield_ph_MeV"], linestyle="none", marker=style["marker"], color=color, markersize=MARKERSIZE + 0.8, markeredgecolor="white", markeredgewidth=0.45)
    _apply_recipe_axes(ax, recipe, default_xlabel="Incident energy [keV]", default_ylabel=r"Primary yield [ph MeV$^{-1}$]", default_title=channel.label)
    apply_axis_style(ax, legend=True, legend_kwargs=_boxed_legend_kwargs(loc=as_text(recipe.get("legend_loc"), "best"), ncol=int(as_int(recipe.get("legend_ncol"), 1) or 1), handlelength=2.1))
...
def _wexc_handles(
    gases: Sequence[WexcGas] = WEXC_GASES,
    particles: Sequence[str] = tuple(PARTICLE_STYLE),
) -> tuple[list[Line2D], list[Line2D]]:
    colors = _wexc_colors()
    gas_handles = [
...
def _combined_wexc_handles(
    gases: Sequence[WexcGas] = WEXC_GASES,
    particles: Sequence[str] = tuple(PARTICLE_STYLE),
) -> list[Line2D]:
    gas_handles, particle_handles = _wexc_handles(gases, particles)
    spacer = Line2D([], [], linestyle="none", marker=None, color="none", label="Incident type")
    return [*gas_handles, spacer, *particle_handles]


def _plot_wexc(wexc: pd.DataFrame, output: Path, recipe: Mapping[str, object]) -> None:
    fig, ax = plt.subplots(figsize=FIGSIZE_WIDE)
    colors = _wexc_colors()
    selected_gas_ids = _selected_ids(recipe, "component", [gas.id for gas in WEXC_GASES])
    selected_particle_ids = _selected_ids(recipe, "particles", list(PARTICLE_STYLE))
    gases = [gas for gas in WEXC_GASES if gas.id in selected_gas_ids]
    for gas in gases:
        color = colors[gas.id]
        for particle in selected_particle_ids:
            style = PARTICLE_STYLE[particle]
            points = wexc.loc[(wexc["gas_mixture"] == gas.gas_mixture) & (wexc["particle"] == particle)].sort_values("energy_kev")
            if points.empty:
                continue
            ax.errorbar(
                points["energy_kev"], points["Wexc_eV"], yerr=points["ErrWexc_eV"],
                color=color, linestyle=style["linestyle"], marker=style["marker"],
                linewidth=LINEWIDTH_MAIN, markersize=MARKERSIZE + 0.5,
                markeredgecolor="white", markeredgewidth=0.45, capsize=2.5,
            )
    _apply_recipe_axes(ax, recipe, default_xlabel="Incident energy [keV]", default_ylabel=r"$W_{\mathrm{exc}}$ [eV]")
    ax.legend(
        handles=_combined_wexc_handles(gases, selected_particle_ids),
        loc=as_text(recipe.get("legend_loc"), "upper right"),
        ncol=int(as_int(recipe.get("legend_ncol"), 1) or 1),
        bbox_to_anchor=(0.985, 0.985), bbox_transform=ax.transAxes,
        **_boxed_legend_kwargs(handlelength=2.1, fontsize=as_float(recipe.get("legend_fontsize"), 9.4)),
    )
    apply_axis_style(ax); ax.margins(x=0.05, y=0.10)
    fig.savefig(output); plt.close(fig)
...
def export_electrons_xray_predictions(
    ...
    *,
    ...
    if make_plots:
        recipes = read_plot_rows("primary", project_root)
        recipes = recipes.loc[recipes["group"] == "electron_xray"]
        recipe_map = {str(row["plot_id"]): row for _, row in recipes.iterrows()}
        legacy_third_continuum = project_root / "primary_predictions" / "plots" / "electrons_xRay" / "primary_yield_electrons_xRay_Ar_3rd.pdf"
        if legacy_third_continuum.exists():
            legacy_third_continuum.unlink()

        plot_id = "primary_yield_electrons_xRay_all"
        if plot_id in recipe_map:
            recipe = recipe_map[plot_id]
            destination = output_path(recipe, project_root); destination.parent.mkdir(parents=True, exist_ok=True)
            _plot_absolute(simulated, dense, destination, recipe)
        plot_id = "primary_yield_electrons_xRay_relative"
        if plot_id in recipe_map:
            recipe = recipe_map[plot_id]
            destination = output_path(recipe, project_root); destination.parent.mkdir(parents=True, exist_ok=True)
            _plot_relative(simulated, dense, destination, recipe)
        plot_id = "primary_Wexc_electrons_xRay_pure_gases"
        if plot_id in recipe_map:
            recipe = recipe_map[plot_id]
            destination = output_path(recipe, project_root); destination.parent.mkdir(parents=True, exist_ok=True)
            _plot_wexc(wexc, destination, recipe)
        channel_lookup = {channel.id: channel for channel in CHANNELS}
        for plot_id, recipe in recipe_map.items():
            if not plot_id.startswith("primary_yield_electrons_xRay_") or plot_id in {
                "primary_yield_electrons_xRay_all", "primary_yield_electrons_xRay_relative"
            }:
                continue
            component_ids = _selected_ids(recipe, "component", ())
            if len(component_ids) != 1 or component_ids[0] not in channel_lookup:
                raise ValueError(f"Individual electron/X-ray plot {plot_id!r} requires exactly one registered channel")
            destination = output_path(recipe, project_root)
            destination.parent.mkdir(parents=True, exist_ok=True)
            _plot_individual(simulated, dense, channel_lookup[component_ids[0]], destination, recipe)
        print(f"[primary_predictions] electron/X-ray plots selected from config/plots/primary.csv")
