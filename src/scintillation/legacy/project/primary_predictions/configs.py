...
from scintillation.registry import ProjectRegistry  # noqa: E402

PROJECT_REGISTRY = ProjectRegistry.load(PROJECT_ROOT)
...
def _ar2nd_degrad_csv(gas_mixture: str) -> Path:
    mixture = PROJECT_REGISTRY.mixture(gas_mixture.replace("-", ""))
    if not mixture.second_continuum:
        raise ValueError(f"The second continuum is disabled for {mixture.mixture_id!r}")
    path = PROJECT_ROOT / "data" / "Primary_DegradData" / f"{mixture.mixture_id}_Ar2nd.csv"
    if not path.exists():
        raise FileNotFoundError(
            f"Missing dedicated second-continuum populations for {mixture.mixture_id}: {path}. "
            "Add the Degrad table; no model-code change is required."
        )
    return path
...
def _ar2nd_component(gas_mixture: str, energy_xray_kev: float | None = None):
    ...
    mixture_spec = PROJECT_REGISTRY.mixture(gas_mixture)
    energy = mixture_spec.default_xray_energy_keV if energy_xray_kev is None else float(energy_xray_kev)
...
def normalization_from_registry(normalization_id: str) -> NormalizationConfig:
    spec = PROJECT_REGISTRY.normalization(normalization_id)
    return NormalizationConfig(
        mode=spec.mode,
        reference_fit_name=spec.reference_fit,
        output_scale=spec.output_scale,
        output_unit=spec.output_unit,
        propagate_nnorm=spec.propagate_nnorm,
    )


OWN_NORM = normalization_from_registry("own")
NO_NORM_MEV = normalization_from_registry("absolute")
COMMON_ARCF4_NORM = normalization_from_registry("ArCF4")
COMMON_ARN2_NORM = normalization_from_registry("ArN2")
ARCF4_PRIMARY_NORM = COMMON_ARCF4_NORM
ARN2_PRIMARY_NORM = COMMON_ARN2_NORM

# Kept as a public compatibility flag. The value is now controlled by
# config/normalizations.csv rather than being duplicated in this module.
PROPAGATE_NNORM_IN_REFERENCE_NORM = COMMON_ARCF4_NORM.propagate_nnorm
...
# =============================================================================
# Declarative production plot registry
# =============================================================================
# The definitions above document the historical configuration and remain useful
# when this module is copied standalone. In the repository, the CSV registry is
# canonical: adding/removing a figure no longer requires editing Python.
from scintillation.plotting.recipe_config import (
    load_primary_band_plots as _load_primary_band_plots_csv,
    load_primary_multiband_plots as _load_primary_multiband_plots_csv,
)


def primary_band_plots(normalization: NormalizationConfig = OWN_NORM) -> list[BandPlotConfig]:
    del normalization  # normalisation is selected per row in config/plots/primary.csv
    return _load_primary_band_plots_csv(
        PROJECT_ROOT,
        "standard",
        band_plot_cls=BandPlotConfig,
        normalization_lookup=normalization_from_registry,
    )


def primary_ir_low_pressure_band_plots(
    normalization: NormalizationConfig = COMMON_ARCF4_NORM,
) -> list[BandPlotConfig]:
    del normalization
    return _load_primary_band_plots_csv(
        PROJECT_ROOT,
...
def arcf4_ir_multiband_plots() -> list[MultiBandPlotConfig]:
    return _load_primary_multiband_plots_csv(
        PROJECT_ROOT,
        "multibar_arcf4_ir",
        curve_cls=BandCurveConfig,
        plot_cls=MultiBandPlotConfig,
        normalization_lookup=normalization_from_registry,
    )
