...
def _registered_primary_runs() -> tuple[DegradRunConfig, ...]:
    project_root = Path(os.environ.get("SCINTILLATION_ROOT", ROOT_DIR.parent))
    path = project_root / "config" / "primary_population_groups.csv"
    if not path.exists():
        return RUNS
    ...
    for run_id, group in frame.groupby("run_id", sort=False):
        ...
        for row in group.itertuples(index=False):
            selections[str(row.display_name)] = [
                [token for token in str(row.name_tokens).split(";") if token], str(row.gas),
                float(row.energy_low_eV), float(row.energy_high_eV), str(row.output_name),
            ]
        dataframe=pd.DataFrame(selections,index=["name principal","gas","energy low","energy up","name output"])
        runs.append(DegradRunConfig(
            name=str(run_id), txt_dir=PRIMARY_DIR/str(first.input_subdir), gas1=str(first.gas1),
            gas2=str(first.gas2), concentration_gas=str(first.concentration_gas), dataframe=dataframe,
            raw_csv_dir=PRIMARY_DIR/str(first.mixture_id)/"csv",
            population_dir=PRIMARY_DIR/str(first.population_subdir),
            output_general_csv=PRIMARY_DIR/str(first.output_file),
        ))
    return tuple(runs)

RUNS = _registered_primary_runs()
