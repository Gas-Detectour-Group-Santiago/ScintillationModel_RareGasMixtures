...
ar_cf4_vis_err = ar_cf4_total_errors_vis[::-1]

#
rate = 2.42
#

ar_cf4_conc_1bar_Florian = np.array([100, 20], dtype=float)
ar_cf4_vis_1bar_Florian = np.array([0.3542 * 187/530,
                                    0.3542 * 530/530
                                ]
                                ,dtype=float) * rate
ar_cf4_vis_err_1bar_Florian = np.array([0.5306257209698674 * ar_cf4_total_errors_vis[-1],                                   
                                3.7942320129687266 * ar_cf4_total_errors_vis[-1]]
                                ,dtype=float) * rate

#ar_cf4_total_errors_vis_Florian = {(20, 1): 3.7942320129687266 * ar_cf4_total_errors_vis[-1], (20, 0.05): 1.132384303590726 * ar_cf4_total_errors_vis[-1], (100, 1): ar_cf4_total_errors_vis[-1], (100, 0.025): 0.5306257209698674 * ar_cf4_total_errors_vis[-1]}


ar_cf4_conc_50mbar_Florian = np.array([100, 20], dtype=float)
ar_cf4_vis_50mbar_Florian = np.array([0.3542 * 45/530,
                                    0.3542 * 130/530,
                                ]
                                ,dtype=float) * rate
ar_cf4_vis_err_50mbar_Florian = np.array([0.5306257209698674 * ar_cf4_total_errors_vis[-1],
                                1.132384303590726 * ar_cf4_total_errors_vis[-1], 
                                ],dtype=float) * rate
...
def select_lip_subset(df, gas_name, case_name, pressure, efield_min, efield_max, gap):
    ...
    """
    mask = (
        np.isclose(df["gap_mm"], gap)
        & (df["electric_field"] < efield_max)
        & (df["electric_field"] > efield_min)
        & np.isclose(df["pressure"], pressure, atol=0.01)
    )
...
# BUILD DATA + THEORY
# ============================================================
ar_theory_1bar_LIP = build_theory_family(
    gas_name="ArCF4",
    pressure=1,
    efield_min=60.0,
    efield_max=300.0,
...
ar_theory_1bar_Florian = build_theory_family(
    gas_name="ArCF4",
    pressure=1,
    efield_min=5.0,
    efield_max=50.0,
...
ar_theory_50mbar_Florian= build_theory_family(
    gas_name="ArCF4",
    pressure=0.050,
    efield_min=5.0,
    efield_max=50.0,
...
ar_theory_25mbar_Florian = build_theory_family(
    gas_name="ArCF4",
    pressure=1,
    efield_min=2,
    efield_max=50.0,
...
if type_of_data == "normalized":
    ...
    ar_cf4_conc_50mbar_Florian = np.array([20,100], dtype=float)

    ar_cf4_efield_50mbar_Florian = np.array([8.421,12.3], dtype=float)
...
series = [
    ...
    },
    {
        ...
        "gas_theory": ar_theory_1bar_Florian,
        "channel": "VIS",
        "color": colors[1],
        "marker": "s",
        "linestyle": "--",
        "xexp": ar_cf4_conc_1bar_Florian,
        "yexp": ar_cf4_vis_1bar_Florian,
        "yerr": ar_cf4_vis_err_1bar_Florian,
    },
    {
        "name": r"50 mbar th-GEM Florian",
        "gas_theory": ar_theory_50mbar_Florian,
        "channel": "VIS",
        "color": colors[2],
        "marker": "^",
        "linestyle": "-.",
        "xexp": ar_cf4_conc_50mbar_Florian,
        "yexp": ar_cf4_vis_50mbar_Florian,
        "yerr": ar_cf4_vis_err_50mbar_Florian,
    },
    # {
    #     "name": r"He-CF$_4$ UV",
...
ax.set_title(r"Ar-CF$_4$ VIS (400-720 nm)")

ax.set_xlim(3, 110)
ax.set_ylim(1e-2, 1.5)

ax.legend(loc="lower left", fontsize=11.2, frameon=True, ncol=2)
