...
def read_parameter_products() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def toys(kind: str) -> np.ndarray:
        path = DATA_DIR / "FitResults" / f"{FIT_NAME}_toys_{kind}.csv"
        if not path.exists():
            return np.empty((0, central.size), dtype=float)
        frame = pd.read_csv(path)
        return frame.to_numpy(dtype=float) if len(frame) else np.empty((0, central.size), dtype=float)

    return central, toys("stat"), toys("syst")
...
def export_overlay_products(
    ...
    # Keep the same visible log-x convention as the old low-pressure figures.
    f_grid = np.geomspace(1e-5, 0.999, 600)
    for mixture in ("ArCF4", "ArN2"):
        fig, ax = plt.subplots(figsize=(7.0, 4.8))
        all_frames: list[pd.DataFrame] = []
        for ip, pressure_mbar in enumerate(PRESSURES_MBAR):
            color = cmap((ip + 0.5) / len(PRESSURES_MBAR))
            ...
            x = frame["concentration_percent"].to_numpy()
            y = frame["value"].to_numpy()
            lo = np.maximum(y - frame["total_minus"].to_numpy(), 1e-30)
            hi = y + frame["total_plus"].to_numpy()
            ax.plot(x, y, lw=1.7, color=color, label=f"{pressure_mbar:g} mbar")
            if np.any(frame["total_plus"].to_numpy() > 0):
                ax.fill_between(x, lo, hi, color=color, alpha=0.12, linewidth=0)

        combined = pd.concat(all_frames, ignore_index=True)
        combined.to_csv(band_dir / f"{FIT_NAME}_{mixture}_total_0p1to1000mbar.csv", index=False)
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_xlim(1e-3, 100.0)
        ax.set_ylim(*Y_LIM)
        gas = r"Ar--CF$_4$" if mixture == "ArCF4" else r"Ar--N$_2$"
        xgas = r"CF$_4$" if mixture == "ArCF4" else r"N$_2$"
        ax.set_xlabel(xgas + " concentration [%]")
        ax.set_ylabel(r"IR yield [ph/MeV]")
        ax.set_title(gas + " joint-fit IR prediction")
        ax.legend(ncol=2, fontsize=8, loc="best")
        fig.tight_layout()
        fig.savefig(plot_dir / f"{FIT_NAME}_{mixture}_total_0p1to1000mbar_overlay.pdf", bbox_inches="tight")
        plt.close(fig)
...
def export_pure_ar_table_and_plot(
    ...
    mean = frame["value_mean"].to_numpy(dtype=float)
    lo = mean - frame["total_minus"].to_numpy(dtype=float)
    hi = mean + frame["total_plus"].to_numpy(dtype=float)
    fig, ax = plt.subplots(figsize=(6.8, 4.6))
    ax.plot(x, frame["arcf4_degrad_value"], "--", lw=1.2, label=r"Ar--CF$_4$ Degrad population")
    ax.plot(x, frame["arn2_degrad_value"], ":", lw=1.4, label=r"Ar--N$_2$ Degrad population")
    ax.plot(x, mean, "o-", lw=1.8, ms=4, label="Joint recommendation")
    if np.any(frame["total_plus"].to_numpy() > 0):
    ...
    ax.set_ylabel(r"Pure-Ar IR yield [ph/MeV]")
    ax.set_title("Pure-Ar IR prediction from the joint fit")
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(plot_dir / f"{FIT_NAME}_low_pressure_pure_ar.pdf", bbox_inches="tight")
    plt.close(fig)
