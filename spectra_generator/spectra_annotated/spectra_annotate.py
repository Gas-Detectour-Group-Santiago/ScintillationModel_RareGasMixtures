...
def resolve_existing_path(path: str | Path, fallback_paths: list[str | Path] | None = None) -> Path:
    ...
    requested = Path(path)
    replacements = {
        "CF4_primary_data_final.pkl": ("CF4_data.pkl", "CF4_data"),
        "CF4_primary_data_final": ("CF4_data.pkl", "CF4_data"),
        "N2_primary_data_final.pkl": ("N2_data.pkl", "N2_data"),
        "N2_primary_data_final": ("N2_data.pkl", "N2_data"),
    }
    if requested.name in replacements:
        for replacement in replacements[requested.name]:
            candidates.extend(_candidate_paths(requested.with_name(replacement)))
    if requested.suffix == "":
        candidates.extend(_candidate_paths(requested.with_suffix(".pkl")))
...
def extract_spectrum_arrays_quiet(
    ...
    fallback_columns = ("mean_spectrum", "C1_spectrum", "C2_spectrum", "data(norm)")
    for col in (*preferred_columns, *fallback_columns):
...
def resolve_column(df: pd.DataFrame, requested: str, path: Path) -> str:
    aliases = {
        "concentracion": ("concentracion", "concentration_CF4", "CF4 concentration (%)"),
        "N2 concentration (%)": ("N2 concentration (%)", "concentration_N2"),
        "CF4 concentration (%)": ("CF4 concentration (%)", "concentration_CF4", "concentracion"),
        "presion": ("presion", "P (bar)"),
        "P (bar)": ("P (bar)", "presion"),
    }
    for col in aliases.get(requested, (requested,)):
        if col in df.columns:
            return col
    raise KeyError(f"Column {requested!r} not found in {path}.")
...
def get_spectrum_from_source(
    ...
    *,
    ...
    concentration_column = resolve_column(df, concentration_column, pkl_path)
    pressure_column = resolve_column(df, pressure_column, pkl_path)
