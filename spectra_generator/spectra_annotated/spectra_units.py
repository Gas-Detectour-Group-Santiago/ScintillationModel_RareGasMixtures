...
def repo_root_from_script(script_file: str | Path) -> Path:
    """
    Return a robust project root for scripts that may be moved.

    Priority:
      1. the first parent containing data/Experimental;
      2. the first parent containing a data directory;
      3. the parent of the script directory.

    This avoids hard-coding a fixed repository layout such as
    ``.../spectra_generator/<script>.py``.
    """
    script_dir = Path(script_file).resolve().parent

    for parent in (script_dir, *script_dir.parents):
        if (parent / "data" / "Experimental").exists():
            return parent
...
def get_arcf4_total_yield_ph_per_electron(row) -> float:
    ...
    total = safe_sum([yz.get("UV"), yz.get("uv"), yz.get("vis")])
    ir = yz.get("ir", {})
...
def get_n2_total_yield_ph_per_electron(row, include_ir: bool = True) -> float:
    ...
    yz = row.get("yields_zonas", {})
    if isinstance(yz, dict) and len(yz) > 0:
        total = safe_sum([yz.get("UV"), yz.get("uv"), yz.get("yield_N2")])
        if include_ir:
            ir = yz.get("ir", {})
            if isinstance(ir, dict):
                total += safe_sum(ir.values())
...
def safe_dill_load(path: str | Path):
    """Load old dill/pickle files robustly."""
    path = Path(path)
