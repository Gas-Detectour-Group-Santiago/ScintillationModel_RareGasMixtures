...
def _normalize_yield_name_for_zonas(yield_name):
    """
    Alias de compatibilidad: los pickles nuevos guardan el yield de N2 como
    yields_zonas["uv"], pero el ajuste primario sigue pidiendo yield_N2.csv.
    """
...
def _get_series_for_yield(
    ...
    if "yields_zonas" in df_pressure.columns:
        zone_yield_name = _normalize_yield_name_for_zonas(yield_name)
        ...
        s = df_pressure.set_index(conc_col)[value_col].apply(
            lambda d: _extract_yield_from_zonas(d, zone_yield_name, yield_mode=yield_mode)
        )
...
def _sanitize_yield_output(df_out, pressure_labels):
    out = df_out.copy()
    for ptag in pressure_labels:
        y_col = f"{ptag}bar"
        e_col = f"Err {ptag}bar"
        if y_col not in out.columns:
            continue

        out[y_col] = _to_numeric_array(out[y_col])
        if e_col in out.columns:
            out[e_col] = np.abs(_to_numeric_array(out[e_col]))

        y = _to_numeric_array(out[y_col])
        bad = ~np.isfinite(y)
        if NEGATIVE_YIELDS_TO_NAN:
            bad |= y < 0.0

        out.loc[bad, y_col] = np.nan
        if e_col in out.columns:
            out.loc[bad, e_col] = np.nan
...
def read_experimental(
    ...
    if output_concentration_name is None:
        if conc_col in {"concentracion", "concentraciones", "CF4 concentration (%)", "concentration_CF4"}:
            output_concentration_name = "fCF4"
        elif conc_col == "concentration_N2":
            output_concentration_name = "N2 concentration (%)"
        else:
    ...
    for y in yields:
        ...
        for p in presiones:
            ...
            if df_pressure.empty:
                yield_out[f"{p:.1f}bar"] = np.nan
                yield_out[f"Err {p:.1f}bar"] = np.nan
                continue
        ...
        yield_out = _sanitize_yield_output(yield_out, [f"{p:.1f}" for p in presiones])
